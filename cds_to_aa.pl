...
cds_to_aa.pl  --  translate nucleic acid sequence with phase information to amino acid sequence, to know whether your input data is complete and correct gene coding sequence (CDS), I recommond you to run "cds_to_aa.pl --check" at first before translate the DNA seqencing into amino acid sequence 
...
(1) translate coding DNA sequence to protein seqeunce. 
	...
	an option "--translate_table" is given for this usage. 
	...
	given in the head line of the fasta format, like: ">gene1_exon2  phase:1". If no phase information is 
	set, then the program regard phase as 0.
...
(2) check the quality of cds sequence. 
    this function only used to the whole cds sequence, not individual exon, check whether it has correct start codon, correct
...
=head1 Command-line Option
  ...
  perl cds_to_aa.pl  <infile.fa | STDIN>
  --translate_table <str>   set the translate table, default=standard
  --check    check the completeness of correctness of whole cds model
  --verbose           output verbose information to screen  
  --help              output help information to screen  

=head1 Usage Exmples

  perl ./cds_to_aa.pl test-data/BMF000017.cds.fa 
  perl ./cds_to_aa.pl test-data/BMF000017.exon.fa 
  perl ./cds_to_aa.pl -check test-data/test_3chrs.fa.fgenesh.100.cds 
...
print "SeqId\tCorrectStartCodon\tCorrectStopCodon\tNoMiddleStopCodon\tTripleBaseLength\n" if($Check);
...
if ($Check && $Verbose){
	print STDERR "wrong_start_codon\twrong_stop_codon\twith_middle_stop_codon\tnot_triple_base_length\n";
