...
getGeneSeq.pl  --  get sequence of gene elements from the genome sequences according to position coordinates

=head1 Description

This program is used to get out the sequence of gene elements, such as exon, intron, mrna, gene, splicing site, 
5'-flanking, 3'-flanking. Both mrna and gene options can be co-used with 5'-flanking and 3'-flanking options.

Input files: position file [psl or gff], and genome file [fasta].

The psl position file can be generated by align the cDNA/EST to the genome using blat, UTR is included;
The gff position file can be generated by de novo gene prediction [mRNA/CDS] or homology protein alignment [match/HSP], UTR is not included; 


The program can detect the position file format [psl or gff] automatically, or you can set by "--posformat" manually.

The default for "--type" option is "mrna", note that for gff positions without UTR, mrna is equal to cds.
...
=head1 Usage
  % $0  [option] <position_file> <sequence_file.fa>
  --posformat <str>   specify format of the position file
...
=head1 Exmple
 ...
 perl ./getGeneSeq.pl chr01.psl chr01.fa -type mrna
 perl ./getGeneSeq.pl chr01.psl chr01.fa -type splice
 perl ./getGeneSeq.pl chr01.psl chr01.fa -type intron
 perl ./getGeneSeq.pl chr01.psl chr01.fa -type exon
 perl ./getGeneSeq.pl chr01.psl chr01.fa -type gene
 perl ./getGeneSeq.pl chr01.psl chr01.fa -type mrna -flank5 50 -flank3 50
...
sub read_gff{
	...
	##sort the exon order based on coordinates on chromosome
