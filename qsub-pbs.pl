...
qsub-pbs.pl -- control processes running on linux PBS system
...
=head1 Usage
  ...
  --queue <str>     specify the queue to use, default low
  --interval <num>  set interval time of checking by qstat, default 1 second
  ...
  --secure <mark>   set the user defined job completition mark, default no
  --reqsub          reqsub the unfinished jobs untill they are finished, default no       
  --resource <str>  set the required resource used in qsub -l option, default mem=1G,nodes=1:ppn=1
...
while (@Shell) {
	...
	my $job_cmd = "qsub  ";  ## -l h_vmem=16G,s_core=8  -q mem.q -P mem
	...
	if($qsub_cycle > 100){
		...
		die "\nProgram stopped because the reqsub cycle number has reached 100\n";
	}
...
sub run_count {
	...
	my $qstat_result = `qstat -a -u $user`;
	...
	foreach my $job_line (@jobs) {
		...
		my $job_id = $1 if($job_field[0] =~ /(\d+)/);
		if (exists $all_p->{$job_id}){
			if (  ($job_field[9] eq "Q" || $job_field[9] eq "R") ) {  
				$run_p->{$job_id} = $job_field[3]; ##job id => shell file name
				$run_num++;
			}
		}
	}
